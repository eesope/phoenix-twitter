...
# live component
  ...
  @impl true
  def render(assigns) do # via HEEx template
    ~H"""
    <div>
      <.simple_form
        for={@form}
        ...
        phx-submit="save"
      >
        <.input field={@form[:body]} type="textarea" label="New Post" />
        <:actions>
  ...
  @impl true
  def update(%{post: post} = assigns, socket) do # take update from index(LiveView)
    {:ok,
     socket
     |> assign(assigns)
     |> assign_new(:form, fn ->
